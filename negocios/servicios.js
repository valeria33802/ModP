const { obtenerdatoscomprador } = require('../datos/repositorios');
...
const {
  ...
  // Función para obtener periféricos (consulta a una vista)
  async function obtenerPerifericosService() {
    const response = await obtenerPerifericos();
    return response;
  }

  const obtenerCompradorLogin = async () => {
    try {
        const [rows] = await obtenerdatoscomprador();
