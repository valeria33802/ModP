...
async function modificarCompradorService(id, ncorreo, ncontrasenia, nnombre, napellido, ndireccion) {
  const response = await sp_ModificarComprador(id, ncorreo, ncontrasenia, nnombre, napellido, ndireccion);
  return response;
}
...
async function modificarEmpleadoService(id, ncorreo, ncontrasenia, npuesto, nnombre, napellido, nhorario) {
  const response = await sp_modificar_empleado(id, ncorreo, ncontrasenia, npuesto, nnombre, napellido, nhorario);
  return response;
}
...
const obtenerPerifericosService = async () => {
  return await repositorio.obtenerperifericos(); // Llamar la función del repositorio
};

const generar_info_compradorService = async () => {
  return await repositorio.sp_generar_info_ultimo_comprador(); // Llamar la función del repositorio
};

const historial_compra_ultimo_usuarioService = async () => {
  return await repositorio.sp_historial_compra_ultimo_usuario(); // Llamar la función del repositorio
};
